"use strict"; var emptycontrol; (function (emptycontrol) {
    ...
    emptycontrol.settings = settings; function fire() {
        buttonDisable(0); if (emptycontrol.initSettings.selector === "" || emptycontrol.initSettings.selector === null) emptycontrol.initSettings.selector = defaultSelector; var required_ec = document.querySelectorAll(emptycontrol.initSettings.selector); for (var i = 0; i < required_ec.length; i++) {
    ...
    function buttonDisable(type = 0) {
        var button = document.querySelector(emptycontrol.initSettings.dependentButton); if (button !== null) button.setAttribute("tabindex", "-1"); if (type === 1) { if (button !== null) button.disabled = !0; return }
        if (button !== null) { button.onmouseover = function () { var allEmptyControlDOM = document.querySelectorAll(emptycontrol.initSettings.selector); for (var i = 0; i < allEmptyControlDOM.length; i++) { detectValues(allEmptyControlDOM[i]) } } }
    }
